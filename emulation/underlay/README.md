...
- `underlay_plan.json`: source/checksum và bandwidth/delay của từng link;
- `underlay_qdisc_initial.json`: chuỗi HTB→NetEm→CoDel trên từng egress;
- `underlay_routing_state.json`: địa chỉ và route OSPF;
- `underlay_reachability.json`: output ping từng cặp loopback;
- `underlay_traffic.json`: iperf3, tcpdump và chênh lệch interface counters;
- `underlay_ditg_traffic.json`: demand SNDlib và số đo D-ITG;
- `underlay_load_response.json`: UDP dưới/trên capacity, RTT khi có tải và
  CoDel drop (chỉ sinh khi có `--load-response`);
- `underlay_qdisc_final.json`: counter qdisc sau tải.
...
| Propagation delay | Haversine × 2,1 / 204.000 km/s, cấu hình NetEm |
...
Không sinh đồ thị từ giá trị tùy ý. Dashboard ở checkpoint sau phải đọc các
artifact runtime này. Configured delay vẫn phải ghi là ước lượng; RTT,
throughput, jitter và loss mới là số đo của lần chạy.
